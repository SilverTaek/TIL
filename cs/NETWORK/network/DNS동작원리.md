# DNS 동작 원리
* 우리가 입력한 도메인 주소를 숫자인 IP 주소로 변환하는 과정이 필요한데 이것을 담당하는 시스템
* 길고 복잡한 IP 주소를 외울수가 없기 때문에 사용
* 도메인 네임 스페이스
    1. 최상위에 루트 DNS 서버가 존재하고, 그 하위로 인터넷에 연결된 모든 노드가 연속해서 이어진 계층구조로 구성
* 네임 서버
    1. 주소를 변환 시키기 위해 도메인 네임 스페이스의 트리구조에 대한 정보가 필요. 이 정보를 가진 서버 도메인 이름을 IP 주소로 변환하는 것을 네임 서비스
* 리졸버
    1. DNS 클라이언트의 요청을 네임 서버로 전달하고 네임 서버로부터 도메인 이름과 IP 주소를 받아 클라이언트에게 제공하는 기능을 수행

★ DNS 동작과정

 ![image](https://user-images.githubusercontent.com/43171179/135474038-3476763b-1f1f-44bc-aeb5-f9339393731e.png)
